...
def fractionalknapsack(W,Items,n):
    ...
    pr = []
    for i in range(n):
        pr.append([Items[i].value/Items[i].weight,i])
    ...
    for i in range(n):
        if(W==0):
            break
        if(Items[pr[i][1]].weight<=W):
            res += Items[pr[i][1]].value
            W -= Items[pr[i][1]].weight
        else:
            res += (W/Items[pr[i][1]].weight)*(Items[pr[i][1]].value)
            W = 0
    return res
